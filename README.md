...
jobs 명령어는 현재 셸 세션에서 실행 중이거나 중지된 모든 작업을 표시
...
jobs 명령어는 셸 세션에서 실행 중인 작업을 관리하고 모니터링하는 데 유용한 도구, 이를 통해 사용자는 백그라운드 및 중지된 작업을 쉽게 확인하고, 필요에 따라 작업을 포그라운드로 가져오거나 백그라운드로 전송 가능 -> 효율적인 작업 관리를 가능하게 하며, 시스템 리소스를 효과적으로 활용
